...
4. **Pattern** – the pattern recomputes automatically. The *Thread* toggle draws the link between each matched pair of stitch holes on the assembled model, fading out over the first half of the explode slider. The 3D view shows pieces in colour, seams (yellow = turned, orange = butted), fold lines (blue) and stitch holes (red). Drag the slider from *Assembled* through *Exploded* to *Flat pattern*. The unfolding is physical: each piece is treated as rigid triangles hinged along their shared edges, every hinge angle relaxes from its 3D value to zero, so a box net swings open along its fold lines and a curved wall unrolls like a carpet while the opened sheet glides rigidly to its place on the pattern; only the last stretch blends in the small forming strain of pieces that need it, so the final state is exactly the flat pattern.
5. **Edit** – tools in the 3D view:
   ...
   - **Holes** – click a stitch hole to delete it together with its matching hole on the other side of the seam; *Restore holes* brings them back.
...
3. **Edge classification** – for every mesh edge the dihedral angle and the width of the adjacent strip give an estimate of the physical bend radius. Edges bending tighter than the leather's minimum bend radius mark a *tight* band; sharp creases (above the crease angle) are allowed as folds only if the leather at that thickness can take a sharp fold without grooving and the crease line is straight (a crease that curves, such as a flat bottom meeting a rounded wall, cannot be folded and becomes a seam).
...
5. **Cut → flatten → refine** – the mesh is cut open along the seam set (vertices duplicated per side, `cutMeshAlongEdges`), each connected piece is flattened with LSCM followed by ARAP iterations (`geometry/flatten.ts`, solved exactly with a banded Cholesky factorisation after reverse Cuthill–McKee ordering so long wrapped strips stay well-conditioned), and the per-triangle principal strains are measured. A piece passes when it has no flipped triangles and its strain stays under the leather's usable stretch. Otherwise it is cut again:
   - interior vertices with concentrated curvature (box corners) get a **dart** to the nearest boundary,
   - smoothly curved pieces are cut into **regular gores**: the fewest equally spaced planes through an axis (the axis the region is most rotationally symmetric about, or a chosen model axis) that bring every gore within the strain limit, rotated so the seams fall on the most curved spots (a pouch's corners, for instance). A ring-shaped piece gets a single closing cut, an equatorial cut is added when gores alone are not enough, and the seam vertices are snapped onto the cutting planes so every gore is identical,
   - with regular seams disabled, they instead get a **through-cut** across their most central / most strained vertex,
   - closed surfaces get a **plane split**.
   Cut paths strongly prefer to run along existing feature edges (creases) rather than across faces. A closed shape made of flat regions joined by foldable creases is unfolded as a single net: a spanning tree of the creases (longest shared edges first) stays as folds and only the remaining creases are cut, so a box becomes one piece stitched along a few edges with no cuts through any face. Softer, stretchier leather therefore yields fewer, larger pieces; firm veg-tan yields more.
6. **Seams & holes** – boundary loops are traced on the cut mesh and paired by original edge, including darts (a slit sewn to itself, labelled D) and closures (the two ends of one piece joined into a ring, labelled J). Stitch holes are placed by 3D arc length along the shared seam curve at the leather's pitch, so both sides always get the same number of holes at the same positions. Rows keep the nominal pitch and stop short of corners the way hand stitchers do: at a corner (a seam meeting another seam, or a seam turning sharply around an edge) the row stops one pitch plus the edge margin from the corner, leaving the corner clear; at a raw edge half a pitch plus the margin. Turned seams put the holes on the stitch line and add the allowance outside it; butted seams put the cut edge on the seam and inset the holes by the edge margin. Dart wedges on turned seams are folded rather than cut (the offset outline is untangled).
