...
4. **Pattern** – the pattern recomputes automatically. The *Thread* toggle draws the link between each matched pair of stitch holes on the assembled model, fading out over the first 30% of the explode slider. The 3D view shows pieces in colour, seams (yellow = turned, orange = butted), fold lines (blue) and stitch holes (red). Drag the slider from *Assembled* through *Exploded* to *Flat pattern*.
