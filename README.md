...
6. **Seams & holes** – boundary loops are traced on the cut mesh and paired by original edge, including darts (a slit sewn to itself, labelled D) and closures (the two ends of one piece joined into a ring, labelled J). Stitch holes are placed by 3D arc length along the shared seam curve at the leather's pitch, so both sides always get the same number of holes at the same positions. Where two seams meet at a corner the rows end on a shared corner hole, inset from both edges, and the pitch is stretched slightly so the last hole lands exactly there; elsewhere rows stop half a pitch plus the inset from the seam end. Turned seams put the holes on the stitch line and add the allowance outside it; butted seams put the cut edge on the seam and inset the holes by the edge margin. Dart wedges on turned seams are folded rather than cut (the offset outline is untangled).
